...
@export var animation_duration: float = 0.05  # 动画持续时间（秒）
@export var click_sound_path: String = ""  # 点击音效的路径
@export var target_scene_path: String = ""  # 跳转场景的路径

# --- 新增：音量大小控制，0.5 代表原来的一半，1.0 代表原声大小 ---
@export_range(0.0, 1.0) var sound_volume: float = 0.5  
...
func play_sound(sound_path: String):
    ...
    if sound_resource is AudioStream:
        ...
        # --- 新增：使用导出的音量变量，将线性音量转换为 Godot 需要的分贝 (dB) ---
